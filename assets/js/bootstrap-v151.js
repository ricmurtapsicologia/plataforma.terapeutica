...
async function loadStore(name,key){try{const rows=await withTimeout(getAllDecrypted(name,key),6000,`Leitura de ${name}`);setStore(name,rows);return{ok:true,name,count:rows.length}}catch(err){console.error('Falha ao carregar store',name,err);setStore(name,[]);window.__rmBootErrors.push(`${name}: ${err.message||err}`);return{ok:false,name,error:err}}}
async function prepareLocalData(pass){
  try{
    dataBanner('Conectando aos dados locais…');
    await withTimeout(openDatabase(),8000,'Abertura do banco local');
    dataBanner('Validando a proteção dos dados…');
    const v=await withTimeout(unlockVault(pass),15000,'Validação dos dados locais');
    runtime.key=v.key;runtime.salt=v.salt;runtime.vaultKnown=true;
    ...
    await Promise.all(priority.map(name=>loadStore(name,v.key)));
    window.__rmRender?.();
    window.__rmUpdateClock?.();
    dataBanner('Carregando informações complementares…');
    const rest=STORE_NAMES.filter(n=>!priority.includes(n));
    await Promise.all(rest.map(name=>loadStore(name,v.key)));
    window.__rmRender?.();
    try{await withTimeout(runMigrations(v.key),15000,'Atualização da estrutura local')}catch(err){console.warn('Migração não concluída',err);window.__rmBootErrors.push(`migração: ${err.message||err}`)}
    window.__rmDataReady=true;runtime.dataReady=true;
    window.__rmRender?.();window.__rmUpdateClock?.();
    dataBanner(window.__rmBootErrors.length?'Plataforma aberta. Alguns dados apresentaram avisos no diagnóstico.':'Dados carregados. Plataforma pronta.',window.__rmBootErrors.length?'info':'success');
    removeDataBanner(2200);
  }catch(err){
    console.error('Falha ao preparar dados locais',err);
    window.__rmBootErrors.push(`dados locais: ${err.message||err}`);
    runtime.dataReady=false;window.__rmDataReady=false;
    dataBanner(`A interface está disponível, mas os dados locais não puderam ser carregados: ${err.message||err}`,'error');
  }
}
async function authenticate(){
  if(authenticating)return;
  const input=document.getElementById('access-password'),pass=input?.value||'';
  if(!pass){setStatus('Digite a senha.','error');return}
  authenticating=true;
  const button=document.querySelector('[data-access-action="login"]');if(button){button.disabled=true;button.textContent='Entrando…'}
  try{
    if(await sha256Hex(pass)!==ACCESS_HASH)throw new Error('Senha incorreta.');
    setStatus('Senha validada. Abrindo interface…');
    runtime.locked=false;runtime.dataReady=false;
    ...
    await startAppShell();
    void prepareLocalData(pass);
  }catch(err){console.error('Falha no acesso',err);runtime.locked=true;showLogin(err?.message||'Não foi possível abrir a plataforma.','error')}
