...
async function hydrateData(){
  for(const name of STORE_NAMES){
    try{
      setStore(name,await getAllDecrypted(name,runtime.key));
    }catch(err){
      console.error('Falha ao carregar área local',name,err);
      setStore(name,[]);
    }
  }
...
function patchAccessView(source){
  ...
  source=source.replaceAll('O cofre foi bloqueado enquanto esta janela estava aberta. Feche a janela, desbloqueie o cofre e tente novamente.','A sessão foi encerrada enquanto esta janela estava aberta. Feche a janela, entre novamente e tente novamente.');
  return source;
}
...
document.addEventListener('click',e=>{
  ...
  if(action==='login'){e.preventDefault();e.stopImmediatePropagation();authenticate()}
  if(action==='toggle'){
...
document.addEventListener('keydown',e=>{
  if(e.key==='Enter'&&e.target.id==='access-password'){
    e.preventDefault();
    authenticate();
  }
},true);
