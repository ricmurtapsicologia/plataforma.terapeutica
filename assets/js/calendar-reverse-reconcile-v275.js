...
const SCAN_STORAGE='rm.google.calendar.reverse-scan.v275';
...
async function reconcileCalendarDates({force=false}={}){
  if(running||runtime.locked||!runtime.dataReady||!runtime.key||!navigator.onLine)return{created:0,linked:0};
  const last=Number(localStorage.getItem(SCAN_STORAGE)||0);
  if(!force&&last&&Date.now()-last<SCAN_COOLDOWN_MS)return{created:0,linked:0};
  const token=await loadToken();
  if(!token)return{created:0,linked:0};
  running=true;
  try{
    ...
    for(const event of events){
      ...
      if(existing){
        if(!existing.calendarEventId){
          existing.calendarEventId=String(event.id);
          existing.calendarImportedAt=nowISO();
    ...
    document.dispatchEvent(new CustomEvent('rm:calendar-reverse-reconciled',{detail:{created,linked,checked:events.length}}));
