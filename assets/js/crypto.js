...
export async function encryptJson(key,value,aad=''){const iv=randomBytes(12);const additionalData=aad?enc.encode(aad):undefined;const cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv,additionalData},key,enc.encode(JSON.stringify(value)));return{iv:bytesToB64(iv),data:bytesToB64(cipher)}}
export async function decryptJson(key,payload,aad=''){const additionalData=aad?enc.encode(aad):undefined;const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv:b64ToBytes(payload.iv),additionalData},key,b64ToBytes(payload.data));return JSON.parse(dec.decode(plain))}
...
export async function encryptWithPin(value,pin){const salt=randomBytes(16),key=await deriveKey(pin,salt,220000),payload=await encryptJson(key,value,'rm-patient-package');return{format:'rm-pin-aesgcm',version:1,salt:bytesToB64(salt),iterations:220000,...payload}}
export async function decryptWithPin(payload,pin){const key=await deriveKey(pin,b64ToBytes(payload.salt),payload.iterations||220000);return decryptJson(key,payload,'rm-patient-package')}
