...
export async function createVault(password){if(password.length<10)throw new Error('Use uma senha com pelo menos 10 caracteres.');const salt=randomBytes(16),key=await deriveKey(password,salt),verifier=await makeVerifier(key);await setMeta('vault',{format:'rm-indexed-vault',version:1,salt:bytesToB64(salt),iterations:310000,verifier,createdAt:new Date().toISOString()});await setMeta('schemaVersion',SCHEMA_VERSION);return{key,salt}}
export async function unlockVault(password){const vault=await getMeta('vault');if(!vault)throw new Error('Nenhum cofre foi criado neste navegador.');const salt=b64ToBytes(vault.salt),key=await deriveKey(password,salt,vault.iterations||310000);try{if(!await verifyKey(key,vault.verifier))throw new Error()}catch{throw new Error('Senha incorreta ou cofre corrompido.')}return{key,salt}}
