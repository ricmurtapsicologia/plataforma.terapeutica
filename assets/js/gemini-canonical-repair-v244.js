...
const DONE_KEY='rm.google.clinical.canonical-repair.v244.done';
const VERSION='2.4.4';
...
function uniqueFirstPatient(value,patients){const key=normalize(value).split(' ')[0];if(key.length<4)return null;const hits=patients.filter(p=>firstName(p)===key);return hits.length===1?hits[0]:null}
function patientFromName(value,patients){const key=normalize(value);if(!key)return null;return patients.find(p=>aliasesOf(p).some(a=>a===key||a.startsWith(key+' ')||key.startsWith(a+' ')))||uniqueFirstPatient(key,patients)}
...
function learnCanonicalAliases(raws){
  ...
  for(const raw of raws){
    ...
    for(const m of text.matchAll(/\(([^)\n]{2,90})\)/g)){
      const patient=patientFromName(m[1],patients);if(!patient)continue;
      const before=normalize(text.slice(Math.max(0,m.index-180),m.index));
      const candidates=labels.filter(l=>before.endsWith(l)||before.includes(' '+l+' ')).sort((a,b)=>{
        const ai=before.lastIndexOf(a),bi=before.lastIndexOf(b);return bi-ai||b.length-a.length;
      });
      const alias=candidates[0];if(alias&&alias.length>=3)learned[alias]=patient.id;
    }
  }
  return learned;
}
function identifyPatient(raw,learned){
  ...
  for(const p of patients){let score=0,evidence='';
    for(const alias of aliasesOf(p)){if(alias.split(' ').length>=2&&labels.some(l=>l===alias||l.startsWith(alias+' ')||alias.startsWith(l+' '))){score=200;evidence='nome do paciente como participante no Gemini';break}}
    if(!score){for(const l of labels){if(learned[l]===p.id){score=190;evidence='identificador explicitamente relacionado ao paciente no Gemini';break}}}
    if(score)rank.push({patient:p,score,evidence});
  }
  rank.sort((a,b)=>b.score-a.score);if(!rank.length)return{patient:null,reason:'Documento sem paciente ativo identificado com segurança.'};if(rank.length>1&&rank[0].score-rank[1].score<20)return{patient:null,reason:'Associação ambígua entre pacientes.'};return rank[0];
}
...
function sameDayManualRecord(patientId,date,fileId){return arr(data.records).find(r=>r?.patientId===patientId&&r?.date===date&&!r?.source?.fileId&&r?.id!==`gmrec_${fileId}`)||null}
...
async function loadCatalog(){let raw=null;try{raw=JSON.parse(localStorage.getItem(CATALOG_STORAGE)||'null')}catch{}if(!raw)return{version:4,lastScanAt:'',lastError:'',filesSeen:0,aliases:{},items:[]};try{const v=await decryptJson(runtime.key,raw,CATALOG_AAD);return v&&Array.isArray(v.items)?{...v,items:v.items}:{version:4,lastScanAt:'',lastError:'',filesSeen:0,aliases:{},items:[]}}catch{return{version:4,lastScanAt:'',lastError:'',filesSeen:0,aliases:{},items:[]}}}
async function saveCatalog(catalog){const encrypted=await encryptJson(runtime.key,{...catalog,version:5,aliasRuleVersion:'2.4.4',items:arr(catalog.items).slice(-700)},CATALOG_AAD);localStorage.setItem(CATALOG_STORAGE,JSON.stringify(encrypted))}
...
async function repair(){
  ...
  try{
    ...
    const catalog=await loadCatalog(),learned=learnCanonicalAliases(fetched.map(x=>x.raw));
    catalog.aliases=learned;catalog.aliasRuleVersion='2.4.4';catalog.aliasesBootstrappedAt=nowISO();catalog.filesSeen=files.length;
    ...
    for(const d of fetched){
      ...
      if(!match.patient||!d.meeting){
        if(isAutoRecord(oldR,d.file.id)){recordDeletes.add(oldR.id);removed++}
      ...
      if(appointment&&(appointment.status==='Cancelada'||appointment.attendanceStatus==='Desmarcou'))appointment=null;
      if(!appointment){
        appointment={id:`gmappt_${d.file.id}`,patientId:patient.id,date:d.meeting.date,time:d.meeting.time,duration:deriveDuration(d.raw),status:'Realizada',attendanceStatus:'Presente',modality:'On-line',recurrence:'Avulso',note:'Sessão confirmada pelas Anotações do Google Meet/Gemini.',createdAt,updatedAt:nowISO(),source:{type:'google-meet-gemini',fileId:d.file.id,url:d.file.webViewLink||'',sourceModifiedAt}};createdAppointments++;
      }else{
        const before=`${appointment.patientId}|${appointment.date}|${appointment.time}|${appointment.status}|${appointment.attendanceStatus}`;
        appointment={...appointment,patientId:patient.id,date:d.meeting.date,time:d.meeting.time,status:'Realizada',attendanceStatus:'Presente',updatedAt:nowISO(),source:{...(appointment.source||{}),type:appointment.source?.type||'google-meet-gemini',fileId:d.file.id,url:d.file.webViewLink||appointment.source?.url||'',sourceModifiedAt},gemini:{...(appointment.gemini||{}),fileId:d.file.id,url:d.file.webViewLink||'',meetingDate:d.meeting.date,meetingTime:d.meeting.time,rawTime:d.meeting.rawTime||d.meeting.time,sourceModifiedAt}};
        if(before!==`${appointment.patientId}|${appointment.date}|${appointment.time}|${appointment.status}|${appointment.attendanceStatus}`)corrected++;
      }
      appointmentWrites.set(appointment.id,appointment);
      let record=oldR,status='materialized';
      const manual=sameDayManualRecord(patient.id,d.meeting.date,d.file.id);
      if(!record&&manual){status='covered'}
      else if(record?.status==='Finalizado'){status='materialized'}
      else{
    ...
    const changed=appointmentWrites.size+recordWrites.size+appointmentDeletes.size+recordDeletes.size;
    if(changed)document.dispatchEvent(new CustomEvent('rm:local-data-changed',{detail:{storeName:'records',kind:'gemini-canonical-repair-v244',at:nowISO(),changed}}));
    window.__rmRender?.();
    toast(`Gemini corrigido: ${createdAppointments} sessão(ões) criada(s), ${createdRecords} prontuário(s) criado(s), ${corrected} vínculo(s) corrigido(s) e ${removed} materialização(ões) indevida(s) removida(s).`,'success');
  }catch(err){console.error('Falha no reparo canônico Gemini v2.4.4',err);toast(err?.message||'Não foi possível corrigir o histórico do Gemini.','error')}
