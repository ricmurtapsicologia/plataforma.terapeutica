...
function captureRedirect(){
  ...
  const expected=sessionStorage.getItem(STATE_STORAGE)||'';
  ...
  try{
    ...
    if(scope&&missing.length)throw new Error('As permissões de Agenda e leitura do Drive não foram concedidas integralmente.');
...
function beginAuthorization(){
  ...
  const id=String(document.getElementById('rm-google-client-id')?.value||localStorage.getItem(CLIENT_STORAGE)||'').trim();
  ...
  const state=`rmws2.${secureRandom()}`;
...
async function persistPending(){
  ...
  let pending=null;
  try{pending=JSON.parse(sessionStorage.getItem(PENDING_STORAGE)||'null')}catch{}
...
function isWorkspaceAction(action){
  return action==='google-calendar-connect'||action==='workspace-connect';
}
document.addEventListener('click',e=>{
  const el=e.target.closest?.('[data-action]');
  if(!el||!isWorkspaceAction(el.dataset.action))return;
  e.preventDefault();
  e.stopImmediatePropagation();
  try{beginAuthorization()}catch(err){toast(err?.message||'Não foi possível iniciar a conexão com o Google Workspace.','error')}
},true);
...
document.addEventListener('rm:data-ready',e=>{
  ...
  if(!hasPending){
    ...
    if(result.startsWith('error:')){
      sessionStorage.removeItem(RESULT_STORAGE);
      queueMicrotask(()=>toast(result.slice(6),'error'));
    }
    return;
  }
  e.stopImmediatePropagation();
  void (async()=>{
    try{
      ...
      if(imported)toast('Google Workspace conectado. Agenda e leitura do Gemini/Drive foram autorizadas neste dispositivo.','success');
    }catch(err){
      sessionStorage.removeItem(PENDING_STORAGE);
      toast(err?.message||'Não foi possível salvar a autorização do Google Workspace.','error');
    }
