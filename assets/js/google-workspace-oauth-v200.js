...
document.addEventListener('click',e=>{const el=e.target.closest?.('[data-action]');if(!el||!isWorkspaceAction(el.dataset.action))return;e.preventDefault();e.stopImmediatePropagation();try{beginAuthorization()}catch(err){toast(err?.message||'Não foi possível iniciar a conexão com o Google Workspace.','error')}},true);
