...
import {performSync} from './secure-sync-v240.js';
...
async function cleanup(){
  ...
  try{
    ...
    document.dispatchEvent(new CustomEvent('rm:legacy-sync-cleaned',{detail:{count:ids.length}}));
...
document.addEventListener('rm:sync-status',e=>{if(e.detail?.status==='conflict')setTimeout(()=>void cleanup(),180)});
