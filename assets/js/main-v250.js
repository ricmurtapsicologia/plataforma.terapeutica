...
window.__rmEntrypointVersion='3.3.0-main-v250';
...
for(const [path,label] of modules){try{await import(`${path}?v=3.3.0&rev=autofix-20260826b`)}catch(err){console.error(`Falha em ${label}`,err);window.__rmPreBootErrors.push(`${label}: ${err?.message||err}`)}}

let auditNormalizeTimer=null;
async function normalizeAuditedDrafts(){
  try{
    const [{data,runtime,nowISO},{putEncrypted}]=await Promise.all([import('./state.js?v=3.3.0&rev=autofix-20260826b'),import('./database.js?v=3.3.0&rev=autofix-20260826b')]);
...
try{await import('./bootstrap-v240.js?v=3.3.0&rev=autofix-20260826b');if(Array.isArray(window.__rmBootErrors)&&window.__rmPreBootErrors.length)window.__rmBootErrors.push(...window.__rmPreBootErrors)}catch(err){console.error('Falha crítica no bootstrap',err);window.__rmPreBootErrors.push(`Bootstrap: ${err?.message||String(err)}`);const box=document.getElementById('rm-boot-fallback');if(box)box.innerHTML=`<strong>Não foi possível iniciar a plataforma.</strong><div class="small">${String(err?.message||err).replace(/[<>]/g,'')}</div>`;document.dispatchEvent(new CustomEvent('rm:boot-failed',{detail:{message:err?.message||String(err)}}))}
