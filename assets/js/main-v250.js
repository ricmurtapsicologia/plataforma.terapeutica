...
const featureModules=[
  ...
  ['./agenda-controller-v330.js','Agenda Controller v3.3',true],
  ['./clinical-session-runtime-v370.js','Sessão clínica · iniciar Meet, cronometrar e encerrar',true],
  ...
  ['./record-persistence-v322.js','Persistência verificada de prontuários',true],
  ['./record-ui-v323.js','Prontuário · retomada e reassociação de sessão',true],
  ['./record-session-binding-v324.js','Prontuário · vínculo canônico de data e horário',true],
  ...
  ['./clinical-data-integrity-v340.js','Integridade clínica, pacotes, duplicidades e prontuários ausentes',true],
...
async function normalizeAuditedDrafts(){
  try{
    const [{data,runtime,nowISO},{putEncrypted}]=await Promise.all([import('./state.js'),import('./database.js')]);
...
try{
  await import('./bootstrap-v240.js');
  ...
  console.error('Falha crítica no bootstrap',err);window.__rmPreBootErrors.push(`Bootstrap: ${err?.message||String(err)}`);
  const box=document.getElementById('rm-boot-fallback');if(box)box.innerHTML=`<strong>Não foi possível iniciar a plataforma.</strong><div class="small">${String(err?.message||err).replace(/[<>]/g,'')}</div>`;
