...
export function renderAppointments(){
  ...
  const title=`${fmtDate(first)} — ${fmtDate(last)}`;
  ...
  const grid=`<div class="week-calendar-wrap"><div class="week-calendar">
    ...
    ${days.map(d=>{const date=isoDate(d),items=list.filter(a=>a.date===date);return`<div class="week-day-column ${date===todayISO()?'today':''}">${items.map(a=>{const p=patientById(a.patientId),top=Math.max(0,minutesFromStart(a.time)),height=Math.max(34,Math.min(Number(a.duration)||50,180));return`<article class="week-event ${eventClass(a.status)}" style="top:${top}px;height:${height}px" data-action="edit-appointment" data-id="${a.id}" title="${esc(p?.name||'Compromisso')} · ${esc(a.time)}"><div class="week-event-time">${esc(a.time)}</div><strong data-sensitive>${esc(p?.preferredName||p?.name||a.title||'Compromisso')}</strong><div class="week-event-meta">${esc(a.modality||'')} · ${esc(a.status||'')}</div>${p?.phone&&a.status!=='Cancelada'?`<button class="week-wa" type="button" data-action="appointment-whatsapp" data-id="${a.id}" title="Lembrar no WhatsApp">WA</button>`:''}</article>`}).join('')}</div>`}).join('')}
  </div></div>`;

  const upcoming=list.filter(a=>a.status!=='Cancelada');
  return pageHead('Agenda',`Semana de ${title}`,`<button class="btn" data-action="new-appointment">Nova sessão</button>`)+
  `<section class="agenda-toolbar card"><div class="flex gap-8 wrap"><button class="btn secondary" data-action="agenda-prev-week">← Semana anterior</button><button class="btn secondary" data-action="agenda-today">Hoje</button><button class="btn secondary" data-action="agenda-next-week">Próxima semana →</button></div><div class="small muted">${upcoming.length} compromisso(s) na semana</div></section>`+
  (list.length?grid:empty('Semana livre','Nenhuma sessão cadastrada nesta semana.',`<button class="btn mt-12" data-action="new-appointment">Agendar sessão</button>`)+grid)+
  `<section class="card mt-16"><div class="card-title"><div><h3 class="mb-0">Sessões da semana</h3><div class="small muted">Lista operacional para edição, calendário e lembretes.</div></div></div>${list.length?`<div class="list">${list.map(a=>{const p=patientById(a.patientId);return`<div class="agenda-item"><div class="agenda-time">${fmtDate(a.date)}<br>${esc(a.time)}</div><div><strong data-sensitive>${esc(p?.name||a.title||'Compromisso')}</strong><div class="small muted">${esc(a.modality||'')} · ${a.recurrence&&a.recurrence!=='Nenhuma'?esc(a.recurrence):'Sessão única'}</div></div><div class="flex gap-8 wrap items-center">${badge(a.status)}<button class="btn ghost" data-action="edit-appointment" data-id="${a.id}">Editar</button>${p?.phone?`<button class="btn ghost" data-action="appointment-whatsapp" data-id="${a.id}">WhatsApp</button>`:''}</div></div>`}).join('')}</div>`:empty('Sem sessões','')}</section>`;
}
