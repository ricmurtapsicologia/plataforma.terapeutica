...
export function renderAppointments(){
  ...
  const grid=`<div class="week-calendar-wrap"><div class="week-calendar"><div class="week-head week-time-head">Horário</div>${days.map((d,i)=>`<div class="week-head ${isoDate(d)===todayISO()?'today':''}"><strong>${DAY_NAMES[i]}</strong><span>${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}</span></div>`).join('')}<div class="week-time-column">${hours.map(h=>`<div class="week-hour">${String(h).padStart(2,'0')}:00</div>`).join('')}</div>${days.map(d=>{const date=isoDate(d),items=list.filter(a=>a?.date===date);return`<div class="week-day-column ${date===todayISO()?'today':''}" data-agenda-date="${date}" title="Clique em um horário vazio para agendar">${items.map(a=>{const p=patientById(a.patientId),top=Math.max(0,minutesFromStart(a.time)),height=Math.max(42,Math.min(Number(a.duration)||50,180)),displayName=patientDisplayName(p,a.title||'Compromisso'),compactName=patientFirstName(p,a.title||'Compromisso'),eventLabel=`T – ${compactName}`,state=sessionState(a),payment=paymentState(a);return`<button class="week-event ${eventClass(a)} payment-${esc(payment.code)}" type="button" style="top:${top}px;height:${height}px" data-action="appointment-open" data-id="${esc(a.id)}" aria-label="Abrir sessão de ${esc(displayName)} às ${esc(a.time||'')} · ${esc(state)} · pagamento ${esc(payment.label)}" title="${esc(displayName)} · ${esc(a.time||'')} · ${esc(state)} · ${esc(payment.label)}"><span class="week-event-time">${esc(a.time||'')}</span><strong data-sensitive>${esc(eventLabel)}</strong>${height>=48?`<span class="week-event-state">${esc(state)}</span>`:''}<span class="week-payment-pill ${esc(payment.code)}">${esc(payment.label)}</span></button>`}).join('')}</div>`}).join('')}</div></div>`;
