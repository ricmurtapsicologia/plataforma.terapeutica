...
function financialModel(global,patientId){
  ...
  for(const appointment of appointments){
    ...
    if(appointmentIsPerformed(appointment)){
      ...
      if(pay.code==='paid'){row.paidSessions++;const linkedPaid=paymentsForAppointment(appointment,{payments:data.payments,appointments:data.appointments}).some(x=>x?.status==='Pago');if(!linkedPaid&&appointment.paymentReconciled===true){row.reconciledOnly+=value}}
      else row.open+=value;
    }else if(appointment.date>todayISO())row.forecast+=value;
  }
  for(const row of rows)row.received+=row.reconciledOnly;
...
function flowChart(model){
  ...
  return`<section class="card mt-16 finance-chart-card"><div class="card-title"><div><h3 class="mb-0">Produzido × recebido × em aberto — ${model.year}</h3><div class="small muted">Produzido considera somente sessões realizadas. Recebido usa lançamentos pagos e conciliações documentais verificadas. Em aberto considera sessões realizadas ainda sem quitação.</div></div><div class="finance-legend" aria-hidden="true"><span><i class="finance-legend-dot produced"></i>Produzido</span><span><i class="finance-legend-dot received"></i>Recebido</span><span><i class="finance-legend-dot open"></i>Em aberto</span></div></div><div class="finance-chart-scroll"><div class="finance-column-chart" role="img" aria-label="${esc(aria)}"><div class="finance-scale"><span>${esc(compactMoney(max))}</span><span>${esc(compactMoney(max/2))}</span><span>R$ 0</span></div><div class="finance-plot">${model.rows.map((x,i)=>{const ph=x.produced>0?Math.max(3,(x.produced/max)*100):0,rh=x.received>0?Math.max(3,(x.received/max)*100):0,oh=x.open>0?Math.max(3,(x.open/max)*100):0;return`<button type="button" class="finance-month ${i===currentMonth?'current':''}" data-finance-month="${model.year}-${String(i+1).padStart(2,'0')}" title="Abrir detalhes de ${x.label}/${model.year}"><div class="finance-bars"><span class="finance-bar produced" style="height:${ph}%"></span><span class="finance-bar received" style="height:${rh}%"></span><span class="finance-bar open" style="height:${oh}%"></span></div><div class="finance-month-label">${x.label}</div></button>`}).join('')}</div></div></div></section>`
}
...
export function renderFinance({global=false}={}){
  const p=selectedPatient();if(!global&&!p)return empty('Paciente não selecionado','Selecione um paciente.');const patientId=p?.id||'',list=arr(data.payments).filter(x=>inScope(x,global,patientId)),model=financialModel(global,patientId),tot=model.totals,month=currentMonthKey(),monthPayments=list.filter(x=>monthKey(x.date)===month).length;
  const warning=tot.reconciledOnly?`<div class="notice warning mt-12">${money(tot.reconciledOnly)} do total recebido deriva de sessões conciliadas documentalmente sem lançamento bancário individual ligado à sessão. A origem permanece identificada para auditoria.</div>`:'';
  return`<section class="grid grid-3 finance-metrics">${metric('Recebido',money(tot.received),tot.reconciledOnly?'Inclui conciliação documental':'Pagamentos efetivos')}${metric('Produzido',money(tot.produced),`${tot.sessions} sessão(ões) realizadas`)}${metric('Em aberto',money(tot.open))}${metric('Previsto',money(tot.forecast),'Sessões futuras válidas no ano')}${metric('Sessões realizadas',String(tot.sessions))}${metric('Ticket médio',money(tot.ticket))}</section>${warning}${flowChart(model)}${forecastChart(global,patientId)}<section class="card mt-16"><div class="card-title"><div><h3 class="mb-0">Lançamentos</h3><div class="small muted">Fluxo de caixa e conciliações administrativas. Pacotes mostram competência e sessões alocadas quando disponíveis.</div></div><button class="btn" data-action="new-payment" ${!global&&!p?'disabled':''}>Novo lançamento</button></div>${list.length?`<div class="table-wrap"><table class="table"><thead><tr><th>Data</th><th>Paciente</th><th>Valor</th><th>Status</th><th>Competência</th><th>Sessões</th><th>Forma</th><th></th></tr></thead><tbody>${[...list].sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(x=>`<tr><td>${fmtDate(x.date)}</td><td data-sensitive>${esc(patientFor(x.patientId)?.name||'—')}</td><td class="money">${money(x.value)}</td><td>${badge(x.status)}</td><td>${esc(x.competence||monthKey(x.date)||'—')}</td><td>${arr(x.appointmentIds).length||x.appointmentId?`<span class="badge">${arr(x.appointmentIds).length||1}</span>`:'—'}</td><td>${esc(x.method||'')}</td><td><button class="btn ghost" data-action="edit-payment" data-id="${x.id}">Editar</button></td></tr>`).join('')}</tbody></table></div>`:empty('Nenhum lançamento','Registre pagamentos e pendências.')}</section>`
}
