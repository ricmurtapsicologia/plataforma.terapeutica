...
const ADAPTERS={
  ...
  icaps:{name:'#name',birth:'#birth-date',application:'#application-date'},
...
function confirmDelivery(detail={}){if(cfg?.productionReady!==true)return;const box=completion();box.hidden=false;document.querySelectorAll('form').forEach(f=>{if(!f.closest('.rm-completion'))f.hidden=true});hideLegacyResults();box.scrollIntoView({behavior:'smooth',block:'start'});window.dispatchEvent(new CustomEvent('rm:screening-completed',{detail:{instrumentId,...detail}}))}
...
async function init(){instrumentId=idFromLocation();if(!instrumentId)return;document.body.classList.add('rm-screening-v2');removeSplash();removeWhatsApp();hideLegacyResults();const r=await fetch(CONFIG,{cache:'no-store'});if(!r.ok)throw new Error(`PUBLIC_EXPERIENCE_V2_${r.status}`);const root=await r.json();cfg=root.instruments?.[instrumentId];if(!cfg)throw new Error('INSTRUMENT_CONFIG_MISSING');document.title=cfg.publicName;insertShell(cfg);hideLegacyPresentation();injectIdentity();removeWhatsApp();blockUnvalidatedActions();completion();installApi();drainPending();new MutationObserver(()=>{removeWhatsApp();hideLegacyResults()}).observe(document.body,{childList:true,subtree:true})}
