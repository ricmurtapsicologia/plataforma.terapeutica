...
async function fetchRemote(token=sync.token){
  ...
  if(meta.content)text=b64ToText(meta.content);else if(meta.download_url){const raw=await fetch(`${meta.download_url}${meta.download_url.includes('?')?'&':'?'}rm=${Date.now()}`,{cache:'no-store'});if(!raw.ok)throw new Error('Não foi possível baixar o cofre criptografado.');text=await raw.text()}else throw new Error('O GitHub não retornou o conteúdo do cofre.');
  const envelope=parseJson(text);if(!envelope||envelope.format!==FORMAT)throw new Error('O arquivo remoto não é um cofre clínico compatível.');return{sha:meta.sha,envelope};
}
async function putRemote(envelope,sha=''){
  if(!sync.token)throw new Error('Autorize este dispositivo no GitHub antes de sincronizar.');const url=`${API_ROOT}/contents/${encodeURIComponent(VAULT_PATH).replace(/%2F/g,'/')}`;
  ...
  if(response.status===409||response.status===422){const err=new Error('O cofre remoto mudou durante a sincronização.');err.code='RETRY';throw err}if(response.status===401||response.status===403)throw new Error('A autorização do GitHub não possui permissão de gravação em Contents.');if(!response.ok)throw new Error(`GitHub respondeu ${response.status} ao gravar o cofre.`);return response.json();
}
...
async function decryptEnvelope(envelope,key=sync.key){if(!key)throw new Error('Chave de sincronização indisponível. Entre novamente na plataforma.');try{return validateSnapshot(await decryptJson(key,{iv:envelope.crypto?.iv,data:envelope.payload},FORMAT))}catch{throw new Error('Não foi possível abrir o cofre remoto com a senha desta plataforma.')}}
async function makeEnvelope(snapshot,revision){if(!sync.key||!sync.salt)throw new Error('Chave de sincronização indisponível.');const encrypted=await encryptJson(sync.key,snapshot,FORMAT);return{format:FORMAT,version:1,revision,updatedAt:iso(),deviceId:sync.deviceId,kdf:{name:'PBKDF2-SHA256',iterations:KDF_ITERATIONS,salt:bytesToB64(sync.salt)},crypto:{name:'AES-256-GCM',iv:encrypted.iv},payload:encrypted.data}}
...
export async function tryBootstrapFromRemote(password){
  if(await vaultExists())return false;let remote;try{remote=await fetchRemote('')}catch(err){console.warn('Consulta inicial do cofre remoto falhou',err);return false}if(!remote)return false;
  const salt=b64ToBytes(remote.envelope.kdf?.salt||''),key=await deriveSyncKey(password,salt,Number(remote.envelope.kdf?.iterations)||KDF_ITERATIONS),snapshot=await decryptEnvelope(remote.envelope,key);
  window.__rmSyncApplying=true;try{await importRawDatabase(snapshot)}finally{window.__rmSyncApplying=false}localStorage.setItem(REVISION_STORAGE,String(remote.envelope.revision||0));saveBaseline(snapshot);localStorage.setItem(DIRTY_STORAGE,'0');return true;
}
async function loadStoredToken(){const saved=parseJson(localStorage.getItem(TOKEN_STORAGE)||'');if(saved&&runtime.key){try{const value=await decryptJson(runtime.key,saved,'github-sync-token');if(value?.token)return String(value.token)}catch{}}const handoff=sessionStorage.getItem(TOKEN_HANDOFF)||'';if(handoff&&runtime.key){try{await storeToken(handoff);sessionStorage.removeItem(TOKEN_HANDOFF);return handoff}catch{}}return''}
...
export async function startSyncSession(password){
  ...
  if(remote){sync.remoteExists=true;sync.remoteRevision=Number(remote.envelope.revision||0);localStorage.setItem(REVISION_STORAGE,String(sync.remoteRevision));sync.salt=b64ToBytes(remote.envelope.kdf?.salt||'');sync.key=await deriveSyncKey(password,sync.salt,Number(remote.envelope.kdf?.iterations)||KDF_ITERATIONS)}else{sync.remoteExists=false;sync.salt=randomBytes(16);sync.key=await deriveSyncKey(password,sync.salt,KDF_ITERATIONS)}
  sync.token=await loadStoredToken();if(sync.remoteExists)setStatus(sync.token?'ready':'pending',sync.token?'Cofre remoto localizado.':'Cofre remoto localizado; autorize este dispositivo para sincronização automática.');else setStatus(sync.token?'needs-init':'pending',sync.token?'Cofre remoto ainda não inicializado. Use os dados deste notebook como base inicial.':'Autorize este dispositivo e depois inicialize o cofre a partir do notebook.');
}
async function adoptRemoteVault(remoteSnapshot,revision){
  sessionStorage.setItem(TOKEN_HANDOFF,sync.token||'');window.__rmSyncApplying=true;try{await importRawDatabase(remoteSnapshot)}finally{window.__rmSyncApplying=false}saveBaseline(remoteSnapshot);localStorage.setItem(DIRTY_STORAGE,'0');localStorage.setItem(REVISION_STORAGE,String(revision||0));setStatus('synced','Base remota recebida. Reabrindo o cofre com a chave sincronizada…');setTimeout(()=>location.reload(),700);return true;
}
async function performSync({manual=false}={}){
  if(sync.busy||runtime.locked||!runtime.dataReady)return false;if(!sync.token){setStatus('pending','Autorize este dispositivo no GitHub.');return false}sync.busy=true;setStatus('syncing','Comparando dados locais e remotos…');
  try{
    for(let attempt=0;attempt<3;attempt++){
      ...
      if(vaultFingerprint(localSnapshot)!==vaultFingerprint(remoteSnapshot)){
        ...
        throw new Error('Este dispositivo possui um cofre local diferente do cofre remoto. A sincronização foi bloqueada para evitar sobrescrita. Gere um backup e escolha uma única base de origem.');
      }
...
async function initializeRemote(){
  ...
  try{if(!sync.token)throw new Error('Autorize este dispositivo no GitHub primeiro.');const existing=await fetchRemote(sync.token);if(existing)throw new Error('Já existe um cofre remoto. Use Sincronizar agora em vez de inicializar.');const local=await exportRawDatabase(),count=meaningfulCount(local);if(count===0)throw new Error('Este dispositivo está sem dados clínicos. A inicialização remota foi bloqueada para evitar uma base vazia.');const revision=1,envelope=await makeEnvelope(local,revision);await putRemote(envelope);sync.remoteExists=true;sync.remoteRevision=revision;saveBaseline(local);localStorage.setItem(DIRTY_STORAGE,'0');localStorage.setItem(REVISION_STORAGE,'1');setStatus('synced',`Base inicial protegida e sincronizada · ${count} registro(s).`);toast('Cofre remoto inicializado com os dados deste notebook.','success')}catch(err){setStatus('error',err.message||'Não foi possível inicializar.');toast(err.message||'Não foi possível inicializar a sincronização.','error')}finally{sync.busy=false}
}
...
function tokenModal(){modal('Autorizar sincronização neste dispositivo',`<div class="notice"><strong>O token fica somente neste dispositivo</strong> e é armazenado criptografado com a chave local da plataforma. Ele não é enviado ao repositório nem incluído nos backups clínicos.</div><div class="field mt-16"><label>GitHub Fine-grained Personal Access Token</label><input id="rm-sync-token-input" class="input" type="password" autocomplete="off" spellcheck="false" placeholder="github_pat_…"></div><div class="small muted mt-12">Use um token restrito ao repositório <strong>${OWNER}/${REPO}</strong>, com permissão de leitura e gravação em <strong>Contents</strong>. Configure a autorização separadamente no notebook e no celular.</div>`,`<button class="btn secondary" data-action="close-modal">Cancelar</button><button class="btn" data-action="sync-save-token">Autorizar dispositivo</button>`,true)}
...
window.addEventListener('online',()=>{if(sync.token&&sync.remoteExists)performSync()});window.addEventListener('focus',()=>{if(sync.token&&sync.remoteExists&&runtime.dataReady)performSync()});
