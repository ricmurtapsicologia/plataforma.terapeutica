...
for(const needle of ['clinical-session-start-flex-v372','createAdHocAppointment','manual-flex-start','Sem sessão aberta hoje','Iniciar nova sessão','rollbackAppointment'])assert.ok(header.includes(needle),`Flexible session header missing ${needle}`);
assert.ok(!header.includes('disabled title="Sem sessão agendada para hoje"'),'No-schedule state must not disable session start');
assert.ok(index.includes('clinical-session-header-v371.js?v=3.7.2&rev=flex-start-20260902'),'Index must cache-bust flexible session header');
