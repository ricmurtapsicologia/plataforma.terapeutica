...
const HARDENING_REV='hardening-20260922';
...
assert.ok(index.includes(`main-v250.js?v=${APP_VERSION}&rev=${HARDENING_REV}`),'Index must cache-bust canonical runtime loader with current hardening key');
assert.ok(!index.includes('clinical-session-header-v371.js'),'Clinical session header must be deferred behind authenticated feature boot');
assert.ok(main.includes('clinical-session-header-v371.js'),'Canonical feature loader must own the clinical session header');
...
assert.ok(main.includes('record-session-binding-v324.js'),'Canonical main must boot record session date/time binding runtime');
assert.ok(main.includes('ui-stability-v380.js'),'Canonical main must boot visual stability runtime');
assert.ok(main.includes('featureStyles'),'Canonical feature loader must defer feature-only styles until authenticated shell');
