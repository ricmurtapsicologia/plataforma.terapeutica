...
for(const term of ['Síntese para revisão','Demanda atual','Recursos e apoio','Objetivos e expectativas'])assert(intake.includes(term),`Anamnese clínica sem ${term}.`);

const sessions=read('assets/js/modules/sessions.js');
assert(sessions.includes("['Preparação','Sessão TCC','Registro','Plano','Continuidade']"),'Fluxo de atendimento deve possuir cinco etapas.');
...
for(const term of ['GMAIL_SEND_SCOPE','gmail.googleapis.com/gmail/v1/users/me/messages/send','application/pdf','Gerar recibo do mês','appointmentIds','WhatsApp + PDF','E-mail + PDF'])assert(delivery.includes(term),`Entrega/financeiro sem ${term}.`);
