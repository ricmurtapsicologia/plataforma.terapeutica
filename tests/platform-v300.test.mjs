...
const recordPersistence=read('assets/js/record-persistence-v322.js');
...
assert.ok(main.includes('record-persistence-v322.js'),'verified record persistence v322 must be active');
assert.ok(recordPersistence.includes("rm:local-data-changed"),'record persistence must notify the private sync manager after verified saves');
assert.ok(recordPersistence.includes('repairMissedRecordSyncOnce'),'record persistence must backfill the dirty signal for records saved before v322');
assert.ok(recordPersistence.includes('recoverLegacyDraft'),'record persistence must recover legacy patient drafts before selecting an older session');
assert.ok(recordPersistence.includes('e.stopImmediatePropagation();queueAutosave()'),'record persistence must own record-field autosave and block the legacy settings autosave');
assert.ok(recordPersistence.includes('window.__rmRender'),'explicit record saves must refresh the patient chart history');
