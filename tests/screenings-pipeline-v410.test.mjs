...
assert.equal(manifest.schemaVersion,'2.0.0','canonical manifest must use v2 schema');
...
assert.match(manifest.experiencePolicy,/Nome completo, Data de nascimento and Data de aplicação do rastreio/i,'canonical identity contract missing');
...
assert.equal(experience.schemaVersion,'2.0.0','public experience must use v2 schema');
...
assert.deepEqual(experience.identity?.required,['Nome completo','Data de nascimento','Data de aplicação do rastreio'],'identity fields must remain canonical');
...
console.log('SCREENINGS_PIPELINE_V500_PASS');
